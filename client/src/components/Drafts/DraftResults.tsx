      ...
      <div className="w-full max-w-5xl bg-middle mt-4 p-4 rounded-md shadow-lg">
        ...
        <div className="space-y-4">
          {Object.keys(draftPicks).map((team) => (
            <div key={team} className="bg-dark p-4 rounded-md shadow-md">
              <h3 className="text-md font-semibold mb-2">{team}</h3>
              {draftPicks[team]?.length ? (
                <div className="space-y-2">
                  {leagueSettings && draftPicks[team]
                    .filter((_golfer, index) => Math.ceil((index + 1) / leagueSettings!.NumberOfTeams!) === currentRound)
                    .map((golfer, index) => (
                        <PlayerData
                        player={golfer}
                        even={index % 2 === 0}
